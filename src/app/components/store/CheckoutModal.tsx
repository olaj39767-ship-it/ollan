...
//   // Preload Flutterwave script when modal opens
//   useEffect(() => {
...
//         setFlwLoading(false);
//         // response.status === "successful" | "completed"
...
//             storeCreditUsed:
//               canUseStoreCredit && useStoreCredit
...
//       onclose: () => {
//         setFlwLoading(false);
//         // User closed modal without paying — no error shown
//       },
//     });
//   }, [
...
//                 disabled={isProcessing || isSubmittingOrder || flwLoading}
...
//                   Your order <span className="font-mono font-semibold text-gray-700">{orderId}</span> has been placed.
//                 </p>
//               </div>
//               <div className="bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 w-full text-left">
//                 <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-1">Amount Paid</p>
...
//                         Secured by <span className="font-bold text-amber-500">Flutterwave</span> — Card, Bank Transfer, USSD
//                       </span>
//                     </div>
...
//               {currentStep === 2
//                 ? "You'll be taken to Flutterwave's secure checkout."
...
const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const handleFlutterwavePayment = useCallback(async () => {
    ...
    FlutterwaveCheckout({
      ...
      amount: finalTotal,
      currency: "NGN",
      // ── Restricted to Bank Transfer and OPay only ──
      ...
      callback: async (response: any) => {
        // response fires when Flutterwave reports success on the client
        if (response.status === "successful" || response.status === "completed") {
          // ── Step 1: verify with OUR backend before trusting it ──
          setFlwLoading(false);
          ...
          try {
            ...
            if (!verified) {
              setPaymentError(
                result.message ||
                  "Payment could not be confirmed by our server. Please contact support."
              );
            }
          } catch {
            setPaymentError(
              "Network error while confirming payment. Please contact support with your transaction ID."
            );
          } finally {
            setVerifyingPayment(false);
          }
          ...
          // ── Step 2: payment confirmed — submit the order ──
          setPaymentSuccess(true);
      ...
      onclose: () => {
        setFlwLoading(false);
        setVerifyingPayment(false);
        // User closed without paying — no error shown
      },
    });
  }, [
    customerInfo, finalTotal, referralCode, promoCode, promoStatus,
